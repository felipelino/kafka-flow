...
This fork only adds a new `GoogleProtobufMessageSerializer`.

### Why a new serializer for Protobuf?

Because the current serializer (`ProtobufMessageSerializer`) can't handle the classes generated by the google compiler. When you try to Serialize an instance of class generated by the Google Compiler an exception happens.

So we created a new one that uses the Google Library to Serialize/Deserialize when the class implements the Google.Protobuf.IMessage interface.
